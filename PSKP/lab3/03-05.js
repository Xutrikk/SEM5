...
function asyncFactorialWithSetImmediate(n, callback) {
    if (n < 0) {
        setImmediate(() => callback(new Error("Факториал отрицательного числа не определен")));
        return;
    }
...
const server = http.createServer((req, res) => {
    ...
    if (parsedUrl.pathname === '/fact' && req.method === 'GET') {
        ...
        asyncFactorialWithSetImmediate(k, (error, factResult) => {
            if (error) {
